...
namespace Wikimedia.Domain
{
    class PageDomain
    {
		...
		public void GetFiles(List<Date> dates)
        {
			String url,destino;
		...
		public List<Date> ObtainDates()
        {
			...
			for (int i = 0; i < 5; i++)
			{
				...
				dateList.Add(new Date(y.ToString(), m.ToString(fmt), d.ToString(fmt), h.ToString(fmt))); ;
				if (h == 0)
				{
					h = 24;
					if (d == 1)
		...
		public List<Page> ReadInfo(Date date)
		{
			String domainCode = "";
			String domainName = "";
			...
			try {
					...
					foreach (string line in lines)
					{
						...
						{
							pageList.Add(new Page(date.hour, valuesD[0], domainName, values[1], Int32.Parse(values[2])));
						}
		...
		public void ObtainAllMaxViewPerPage (List<Date> dates)
        {
			List<Page> viewMaxCount = new List<Page>();
			...
			foreach (Page element in viewMaxCount)
			{
		...
		public void ObtainAllMaxViewLD(List<Date> dates)
		{
			List<Page> viewMaxCount = new List<Page>();
		...
		public Page ObtaiMaxViewPerPage(List<Page> pageList)
		{
            ...
            {
				temp.Add(new Page(item.Key.period,"","",item.Key.page,item.viewCount));

			}

			List<Page> pageOrderByViewDesc = temp.OrderByDescending(x => x.viewCount).ToList();
			Page maxView = pageOrderByViewDesc[0];
			return maxView;
			
		}


		public Page ObtainMaxViewCountLD(List<Page> pageList)
        {
			...
			foreach (var item in query)
			{
				temp.Add(new Page(item.Key.period, item.Key.language,item.Key.domain, "", item.viewCount));
